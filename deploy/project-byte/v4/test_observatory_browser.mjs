...
export async function verifyObservatory({page,api,base,out}){
  ...
  const raw=JSON.stringify(data);for(const secret of ['PRIVATE_BROWSER_SENTINEL','SENSITIVE_RESPONSE_BODY','PRIVATE_PROMPT'])assert.ok(!raw.includes(secret));
  ...
  await page.locator('[data-obs-tools="pills"]').click();await page.locator('#obsTools [data-obs-event]').first().click();assert.match(await page.locator('#obsInspector pre').textContent(),/argument_fields/);
  assert.ok(!(await page.locator('#obsInspector').textContent()).includes('PRIVATE_BROWSER_SENTINEL'));
