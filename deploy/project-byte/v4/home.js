...
(() => {
  ...
  function ingest(list,announce=true){
    const added=[];
    for(const raw of list){if(!raw||typeof raw!=='object'||!raw.id||known.has(raw.id))continue;const row={id:text(raw.id),ts:Number(raw.ts)||Date.now()/1000,title:text(raw.title).slice(0,200),message:text(raw.message).slice(0,50000),project:text(raw.project).slice(0,100),conversationId:/^[a-zA-Z0-9_-]{1,100}$/.test(raw.conversationId||'')?raw.conversationId:'',link:/^#[a-zA-Z]+$/.test(raw.link||'')?raw.link:'',source:text(raw.source),read:false};known.add(row.id);records.push(row);added.push(row);}
